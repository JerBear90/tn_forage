...
export default function NewRoutePage() {
  ...
  return (
    <div className="p-4 pb-24">
      <h1 className="text-xl font-bold text-gray-800 mb-4">Plan a Route</h1>
      ...
      <div className="mb-4">
        <h2 className="text-sm font-semibold text-gray-700 mb-2">Waypoints</h2>
        {waypoints.map((wp, i) => (
          <div key={wp.id} className="flex items-center gap-2 mb-2 rounded border border-gray-100 p-2">
            <span className="flex h-6 w-6 items-center justify-center rounded-full bg-teal-100 text-xs font-bold text-teal-700">
              {i + 1}
            </span>
            <span className="flex-1 text-sm text-gray-700">{wp.label}</span>
          </div>
        ))}
