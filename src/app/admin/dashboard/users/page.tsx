...
import { searchUsers, changeUserRole, toggleUserStatus } from '@/services/admin/userService';
...
export default function UsersManagementPage() {
  ...
  const handleRoleChange = (user: AdminUser, newRole: string) => {
    setConfirmDialog({
      ...
      message: `Are you sure you want to change ${user.name || user.email}'s role from "${user.role}" to "${newRole}"?`,
      onConfirm: async () => {
  ...
  const handleToggleStatus = (user: AdminUser) => {
    ...
    setConfirmDialog({
      ...
      message: willDisable
        ? `Are you sure you want to disable ${user.name || user.email}'s account? They will not be able to log in until re-enabled.`
        : `Are you sure you want to re-enable ${user.name || user.email}'s account?`,
      onConfirm: async () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {result && (
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {result.totalCount} total user{result.totalCount !== 1 ? 's' : ''}
          </p>
        )}
      </div>
      ...
      <div className="overflow-x-auto rounded-lg border border-brand-charcoal/10 dark:border-brand-sand/10">
        <table className="w-full text-sm text-left" role="table" aria-label="User management table">
          <thead>
            <tr className="border-b border-brand-charcoal/10 bg-brand-sand/30 dark:border-brand-sand/10 dark:bg-brand-charcoal/50">
              <th scope="col" className="px-4 py-3">
                <button
                  ...
                  className="min-h-[44px] min-w-[44px] font-semibold text-brand-charcoal dark:text-brand-sand hover:text-brand-teal dark:hover:text-brand-teal"
              ...
              <th scope="col" className="px-4 py-3">
                <button
                  ...
                  className="min-h-[44px] min-w-[44px] font-semibold text-brand-charcoal dark:text-brand-sand hover:text-brand-teal dark:hover:text-brand-teal"
                  aria-label={`Sort by last active${getSortIndicator('lastActiveAt') || ''}`}
                >
                  Last Active{getSortIndicator('lastActiveAt')}
                </button>
              </th>
              <th scope="col" className="px-4 py-3 font-semibold text-brand-charcoal dark:text-brand-sand">
                Sessions
          ...
          <tbody>
            {loading && !result ? (
              ...
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                  No users found{debouncedQuery ? ` matching "${debouncedQuery}"` : ''}.
                </td>
              </tr>
            ) : (
              result?.users.map((user) => (
                ...
                >
                  <td className="px-4 py-3 text-brand-charcoal dark:text-brand-sand font-medium whitespace-nowrap">
                    {user.name || '—'}
                  </td>
                  <td className="px-4 py-3 text-brand-charcoal dark:text-brand-sand whitespace-nowrap">
                    {user.email}
                  </td>
                  <td className="px-4 py-3">
                    ...
                    <select
                      ...
                      className="min-h-[44px] rounded border border-brand-charcoal/20 bg-white px-2 py-1 text-sm text-brand-charcoal dark:border-brand-sand/20 dark:bg-brand-charcoal dark:text-brand-sand focus:border-brand-teal focus:outline-none focus:ring-2 focus:ring-brand-teal/30"
                    >
                      {ROLES.map((role) => (
                    ...
                    <button
                      ...
                      aria-label={
                        user.accountStatus === 'active'
                          ? `Disable account for ${user.name || user.email}`
                          : `Enable account for ${user.name || user.email}`
                      }
                      className={`min-h-[44px] min-w-[44px] rounded-lg px-3 py-2 text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                        user.accountStatus === 'active'
      ...
      {result && result.totalPages > 1 && (
        <nav className="flex items-center justify-between" aria-label="User list pagination">
          <p className="text-sm text-gray-500 dark:text-gray-400">
      ...
      <div className="mx-4 w-full max-w-md rounded-xl bg-white p-6 shadow-xl dark:bg-brand-charcoal">
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button
            ...
            className="min-h-[44px] min-w-[44px] rounded-lg bg-brand-teal px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-brand-teal/90 focus:outline-none focus:ring-2 focus:ring-brand-teal/30 disabled:opacity-50"
          >
