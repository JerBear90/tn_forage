...
export default function SeasonalCountdown({ entries }: SeasonalCountdownProps) {
  ...
  return (
    <div className="rounded-lg border border-brand-charcoal/10 dark:border-brand-sand/10 p-4">
      <h3 className="text-sm font-semibold text-brand-charcoal dark:text-brand-sand mb-3">Season Countdown</h3>
        ...
        {entries.map((entry) => (
          <li key={entry.speciesId} className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-brand-charcoal dark:text-brand-sand">{entry.commonName}</p>
              {entry.isInSeason ? (
                <p className="text-xs text-green-600 font-medium">🟢 In season now</p>
              ) : (
                <p className="text-xs text-brand-charcoal/60 dark:text-brand-sand/60">{entry.daysRemaining} days until season</p>
              )}
