...
- **Pre-selection from map** — "Plan a Visit" from map pre-selects the park and collapses the picker
- **Share prompt** — After saving, prompts user to share the trip with the community
...
- **Admin Dashboard link** — Visible only to super_user accounts
...
- **Analytics Dashboard** (`/admin/dashboard`) — Comprehensive admin analytics and management
  - **Overview** — Page views, active users, session duration, error count, Recharts charts, quick actions
  - **Users** (`/admin/dashboard/users`) — Searchable user table, role change, disable/enable accounts
  - **Notifications** (`/admin/dashboard/notifications`) — Push notification composer with 8 templates, scheduling, targeting (all/region/role), 5-min duplicate prevention
  - **Content** (`/admin/dashboard/content`) — Sighting moderation (approve/flag/remove) + blog article CRUD with audit logging
  - **Retention** (`/admin/dashboard/retention`) — DAU/WAU/MAU, churn rate, return rate, cohort retention table
  - **Funnels** (`/admin/dashboard/funnels`) — Conversion funnel tracking (identification + onboarding flows), <20% conversion flagging
  - **Search** (`/admin/dashboard/search`) — Top 20 terms, zero-result searches, click-through rate, content gap detection
  - **Onboarding** (`/admin/dashboard/onboarding`) — 5 milestone completion tracking, time-to-milestone, <30% flagging
  - **Revenue** (`/admin/dashboard/revenue`) — MRR, subscriber count, conversion rate, churn, Stripe dashboard link
  - **Alerts** (`/admin/dashboard/alerts`) — Anomaly detection (error spikes, traffic drops, connection failures), configurable thresholds, auto-resolution
  - **Releases** (`/admin/dashboard/releases`) — Version history with filterable release notes
  - **Reviews** (`/admin/dashboard/reviews`) — User review browser with type/rating filters
  - **Feedback** (`/admin/dashboard/feedback`) — Bug reports, suggestions, and other feedback with device info
  - **Blog** (`/admin/dashboard/blog`) — Full blog management (create, edit, delete, publish/unpublish)
  - **AI Insights** (`/admin/dashboard/ai`) — AI content generator with seasonal/SEO suggestions, publish-to-destination, cross-update notifications, strategic insights
  - **Settings** (`/admin/dashboard/settings`) — LLM config (OpenAI/Claude), SendGrid/Resend email, data retention, security, sample data seeder
  - **Audit Log** (`/admin/dashboard/audit`) — Admin action history (moderation + notifications)
...
- **Notification signup prompt** — First-time users prompted to enable email/SMS notifications
- **Homepage customizer** — Drag-to-reorder sections with "Customize" label
...
│   │   └── admin/        # Admin dashboard services (analytics, alerts, export, etc.)
│   ├── styles/            # Global CSS
